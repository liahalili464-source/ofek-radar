...
import { DemoCycleSimulator } from "@/components/demo-cycle-simulator";
...
export default async function MaintenancePage() {
  ...
  const { data: profile } = await supabase.from("profiles").select("username,role,active").eq("id", user.id).single();
  ...
  return <AppShell title="תחזוקת מערכת" subtitle="גישת IT פרטית למסכי המערכת ולתצוגות המועמד">
    <DemoCycleSimulator />

    <section className="card" style={{ marginBottom: 18 }}>
      <div className="row between wrap" style={{ marginBottom: 14 }}><div><h2 className="section-title" style={{ marginBottom: 4 }}>תצוגות מועמד</h2><div className="stat-label">אפשר להשתמש בקוד 1905 כדי לפתוח את מסכי המועמד במצב תצוגת IT.</div></div><span className="badge ok">גישת IT</span></div>
      <div className="grid grid-2">{publicScreens.map((screen) => <Link key={screen.href} href={screen.href} target="_blank" className="notice" style={{ display: "block", padding: 18 }}><b style={{ display: "block", fontSize: 17, marginBottom: 5 }}>{screen.title}</b><span className="stat-label">{screen.description}</span></Link>)}</div>
    </section>
