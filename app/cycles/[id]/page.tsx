...
export default function CycleSummaryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AppShell
      ...
      actions={<div className="row wrap"><Link href={`/schedule?cycle=${id}`} className="btn btn-primary">שיבוץ ראיונות</Link><Link href="/cycles" className="btn">כל המחזורים</Link></div>}
    >
      ...
      {!cycle ? <div className="empty">לא ניתן להציג את המחזור.</div> : <>
        <div className="grid grid-4" style={{ marginBottom: 18 }}>
          ...
          <StatCard label="שאלונים הושלמו" value={responseCandidateIds.size} accent />
        ...
        <div className="grid grid-2">
          <section className="card">
            <div className="row between" style={{ marginBottom: 16 }}><div><h2 className="section-title" style={{ marginBottom: 4 }}>תמונת מצב</h2><div className="stat-label">התקדמות לפי ראיונות שבוצעו</div></div><StatusBadge status={statusLabel(cycle.status)} /></div>
            <div className="row between"><b>{progress}%</b><span className="stat-label">{Math.max(0, totalInterviews - completedInterviews)} ראיונות נשארו</span></div>
          ...
          <section className="card">
            <div className="row between"><h2 className="section-title">יחידות במחזור</h2><Link href={`/schedule?cycle=${id}`} className="btn btn-small">עריכת שיבוץ</Link></div>
            <div className="grid grid-2">
              {units.map((u) => <div className="notice" key={u.unit_id}><b>{one(u.units)?.name || "יחידה"}</b></div>)}
              {!units.length && <div className="empty">עדיין לא נבחרו יחידות למחזור.</div>}
            </div>
          </section>
        </div>
