...
type Evaluation = { interview_id: string; professional_score: number | null; personal_score: number | null; recommendation: string | null; notes: string | null };
type AttentionFilter = "all" | "questionnaire" | "evaluation";
type AttentionItem = { key: string; candidateId: string; candidateName: string; type: "questionnaire" | "evaluation"; severity: "danger" | "warning"; text: string; startsAt: string };
...
export default function CycleSummaryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const unitProgress = useMemo(() => units.map((unit) => {
    ...
    const lateEvaluations = unitInterviews.filter((x) => new Date(x.starts_at).getTime() < nowMs && !evaluationInterviewIds.has(x.id) && x.status !== "no_show").length;
    return { unitId: unit.unit_id, name: one(unit.units)?.name || "יחידה", total, completed, remaining: Math.max(0, total - completed - noShows), noShows, evaluations: unitEvaluations, lateEvaluations, interviewProgress, evaluationProgress };
  }), [units, interviews, evaluationInterviewIds, nowMs]);

  const upcoming = useMemo(() => interviews.filter((x) => x.status === "scheduled" && new Date(x.starts_at).getTime() >= nowMs).slice(0, 8), [interviews, nowMs]);
  ...
  const attentionItems = useMemo<AttentionItem[]>(() => {
    ...
    for (const interview of interviews) {
      ...
      const starts = new Date(interview.starts_at).getTime();
      const candidateName = one(interview.candidates)?.full_name || "מועמד/ת";

      if (starts < nowMs && !evaluationInterviewIds.has(interview.id)) {
        items.push({ key: `eval-${interview.id}`, candidateId: interview.candidate_id, candidateName, type: "evaluation", severity: "danger", text: `הראיון עם ${one(interview.units)?.name || "היחידה"} עבר וחוות הדעת טרם מולאה`, startsAt: interview.starts_at });
      }

      if (!responseCandidateIds.has(interview.candidate_id) && starts <= within48h) {
        const severity = starts < nowMs ? "danger" : "warning";
        items.push({ key: `questionnaire-${interview.candidate_id}-${interview.id}`, candidateId: interview.candidate_id, candidateName, type: "questionnaire", severity, text: starts < nowMs ? "מועד הראיון הגיע והשאלון עדיין חסר" : `הראיון קרוב (${formatDateTime(interview.starts_at)}) והשאלון עדיין חסר`, startsAt: interview.starts_at });
      }
    }

    const seen = new Set<string>();
    return items.sort((a, b) => (a.severity === b.severity ? +new Date(a.startsAt) - +new Date(b.startsAt) : a.severity === "danger" ? -1 : 1)).filter((item) => {
      const dedupe = `${item.type}|${item.candidateId}`;
      if (item.type === "questionnaire" && seen.has(dedupe)) return false;
      seen.add(dedupe);
      return true;
    });
  }, [interviews, evaluationInterviewIds, responseCandidateIds, nowMs]);
  ...
  return (
    <AppShell
      ...
      actions={<div className="row wrap"><button className="btn" onClick={exportCycleReport}>ייצוא דוח Excel</button><Link href={`/schedule?cycle=${id}`} className="btn btn-primary">שיבוץ ראיונות</Link><Link href={`/cycles/new?edit=${id}`} className="btn">עריכת מחזור</Link><Link href="/cycles" className="btn">כל המחזורים</Link></div>}
    >
