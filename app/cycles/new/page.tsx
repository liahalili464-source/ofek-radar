...
export default function NewCyclePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (currentEditId) {
        ...
        if (editError || !cycleRes.data) {
          if (!cancelled) { setError(editError?.message || "לא ניתן לטעון את המחזור"); setLoading(false); }
          return;
        }
  ...
  const input = useMemo(() => ({
    candidateNames: Array.from({ length: candidateCount }, (_, i) => `candidate-${i}`),
    units: selectedUnitIds,
    days,
    durationMinutes: duration,
  }), [candidateCount, selectedUnitIds, days, duration]);
  ...
  function addDay() {
    setDays((current) => {
      const nextDate = current.length
        ? addDays(current[current.length - 1].date, 1)
        : (startsOn || new Date().toISOString().slice(0, 10));
      return [...current, { date: nextDate, start: "09:00", end: "16:00", breaks: [{ start: "12:30", end: "13:00" }] }];
    });
  }
  ...
  return (
    <AppShell title={title} subtitle={subtitle}>
      ...
      <div className="grid grid-2">
        ...
        <div className="grid" style={{ alignContent: "start" }}>
          <section className="card">
            ...
            {status === "completed" && <div className="notice warning" style={{ marginTop: 14 }}>מחזור סגור נשמר לצפייה היסטורית. כדי לשנות שיבוץ או יחידות, החזירי אותו קודם לסטטוס פעיל.</div>}
          </section>
          ...
          <section className="card">
            ...
            <div className="grid">
              {days.map((day, index) => <div className="notice" key={`${day.date}-${index}`}>
                ...
                <div className="row between"><div className="stat-label">הפסקה: 12:30–13:00</div><button className="btn btn-small btn-danger" disabled={status === "completed"} onClick={() => removeDay(index)}>הסרה</button></div>
              </div>)}
      ...
      <div className="row wrap" style={{ justifyContent: "flex-start", marginTop: 20 }}>
        {editId ? <>
          <button className="btn btn-primary" disabled={saving} onClick={() => saveCycle("summary")}>{saving ? "שומר..." : status === "completed" ? "שמירת המחזור כסגור" : "שמירת שינויים"}</button>
          {status !== "completed" && <button className="btn" disabled={saving || !report.canGenerate} onClick={() => saveCycle("schedule")}>שמירה והמשך לשיבוץ</button>}
        </> : <>
          {status !== "completed" && <button className="btn btn-primary" disabled={saving || !report.canGenerate} onClick={() => saveCycle("schedule")}>{saving ? "שומר..." : "שמירת המחזור והמשך לשיבוץ"}</button>}
          <button className={status === "completed" ? "btn btn-primary" : "btn"} disabled={saving} onClick={() => saveCycle("summary", status !== "completed")}>{status === "completed" ? "שמירת מחזור סגור" : "שמירה כטיוטה"}</button>
        </>}
      </div>
    </AppShell>
  );
