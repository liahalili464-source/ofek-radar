...
export default function SchedulePage() {
  ...
  useEffect(() => {
    ...
    async function loadInitial() {
      ...
      if (!cancelled) {
        setCycles(cycleRows); setUnits((unitsRes.data || []) as Unit[]); setAccounts((accountsRes.data || []) as UnitAccount[]);
  ...
  useEffect(() => {
    ...
    async function loadCycle() {
      ...
      const currentDays = ((dayRes.data || []) as ExistingDay[]).map((d) => ({ date: d.interview_date, start: d.starts_at.slice(0, 5), end: d.ends_at.slice(0, 5), breaks: [{ start: "12:00", end: "13:00" }] }));
      if (!cancelled) { setCandidates(candidateRows); setSelectedUnitIds(((unitRes.data || []) as ExistingCycleUnit[]).map((x) => x.unit_id)); setDays(currentDays); setLoading(false); }
    }
    loadCycle();
    return () => { cancelled = true; };
  }, [cycleId, cycles]);
  ...
  function addDay() {
    ...
    setDays((current) => [...current, { date, start: "09:00", end: "16:00", breaks: [{ start: "12:00", end: "13:00" }] }]); setSaved(false);
  }
  function updateDay(index: number, patch: Partial<InterviewDay>) { setDays((current) => current.map((d, i) => i === index ? { ...d, ...patch } : d)); setSaved(false); }
  ...
  async function saveSchedule() {
    ...
    try {
      ...
      if (days.length) { const { error: insertDaysError } = await supabase.from("interview_days").insert(days.map((d) => ({ cycle_id: cycleId, interview_date: d.date, starts_at: d.start, ends_at: d.end }))); if (insertDaysError) throw insertDaysError; }
  ...
  return (
    <AppShell title="שיבוץ ראיונות" subtitle="בחירת מחזור ויחידות, בדיקת קיבולת ויצירת לו״ז ללא התנגשויות">
      ...
      <section className="card" style={{ marginBottom: 18 }}><div className="grid grid-3"><div className="field"><label>מחזור</label><select className="select" value={cycleId} onChange={(e) => setCycleId(e.target.value)}>{cycles.length === 0 && <option value="">אין מחזורים</option>}{cycles.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div><div className="field"><label>מועמדים במחזור</label><div className="preview-field"><b>{candidates.length}</b> מועמדים</div></div><div className="field"><label>משך ראיון</label><select className="select" value={duration} onChange={(e) => { setDuration(Number(e.target.value)); setSaved(false); }}><option value={20}>20 דקות</option><option value={30}>30 דקות</option><option value={45}>45 דקות</option><option value={60}>60 דקות</option></select></div></div></section>

      <section className="card" style={{ marginBottom: 18 }}><div className="row between"><h2 className="section-title" style={{ marginBottom: 4 }}>יחידות במחזור</h2><span className="badge">{selectedUnits.length} נבחרו</span></div><div className="grid grid-4" style={{ marginTop: 14 }}>{units.map((unit) => { const hasAccount = accountByUnit.has(unit.id); const selected = selectedUnitIds.includes(unit.id); const color = selected ? unitColorById.get(unit.id) : undefined; return <label className="notice checkbox-row" key={unit.id} style={{ opacity: hasAccount ? 1 : .55, borderColor: color || undefined, background: color ? `${color}16` : undefined }}><input type="checkbox" checked={selected} disabled={!hasAccount} onChange={() => toggleUnit(unit.id)} /><span><b>{unit.name}</b>{selected && color && <span style={{ display: "inline-block", width: 9, height: 9, borderRadius: 999, background: color, marginInlineStart: 8 }} />}{!hasAccount && <div className="stat-label">חסר חשבון יחידה</div>}</span></label>; })}</div></section>

      <section className="card" style={{ marginBottom: 18 }}><div className="row between"><h2 className="section-title" style={{ marginBottom: 4 }}>ימי ראיונות</h2><button className="btn btn-small" onClick={addDay}>+ הוספת יום</button></div><div className="grid grid-3" style={{ marginTop: 14 }}>{days.map((day, index) => <div className="notice" key={`${day.date}-${index}`}><div className="grid grid-3"><div className="field"><label>תאריך</label><input className="input" type="date" value={day.date} onChange={(e) => updateDay(index, { date: e.target.value })} /></div><div className="field"><label>התחלה</label><input className="input" type="time" value={day.start} onChange={(e) => updateDay(index, { start: e.target.value })} /></div><div className="field"><label>סיום</label><input className="input" type="time" value={day.end} onChange={(e) => updateDay(index, { end: e.target.value })} /></div></div><div className="grid grid-2"><div className="field"><label>תחילת הפסקה</label><input className="input" type="time" value={day.breaks?.[0]?.start || "12:00"} onChange={(e) => updateBreak(index, "start", e.target.value)} /></div><div className="field"><label>סיום הפסקה</label><input className="input" type="time" value={day.breaks?.[0]?.end || "13:00"} onChange={(e) => updateBreak(index, "end", e.target.value)} /></div></div><button className="btn btn-small btn-danger" onClick={() => removeDay(index)}>הסרת יום</button></div>)}{!days.length && <div className="empty">עדיין לא הוגדרו ימי ראיונות למחזור.</div>}</div></section>

      <div className="grid grid-4" style={{ marginBottom: 18 }}><StatCard label="סה״כ ראיונות" value={report.totalInterviews} /><StatCard label="זמני ראיון נדרשים" value={report.requiredRounds} /><StatCard label="זמני ראיון זמינים" value={report.availableRounds} accent /><StatCard label="מצב" value={<span style={{ fontSize: 21, color: report.canGenerate ? "var(--success)" : "var(--warning)" }}>{report.canGenerate ? "אפשר לשבץ" : "נדרשות התאמות"}</span>} /></div>
